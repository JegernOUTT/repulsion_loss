...
def repulsion_term_box(y_pred, betta):
    # Делим все множество y_pred боксов на бокс + цель
    # (Проходимся циклом и оставляем для каждой y_true бокс из y_pred с наибольшим IoU)
...
def create_repulsion_loss(objectness_score_treshold=0.5, alpha=0.5, betta=0.5):
    def _filter_predictions(y_pred):
        y_pred_indices = tf.where(tf.greater_equal(y_pred[..., 4], objectness_score_treshold))
